...
declare
  ...
  v_post_revision public.blog_post_revisions;
begin
  for v_page in
  ...
  loop
    begin
      v_revision := public.publish_page(v_page.id);
      content_type := 'page';
      id := v_page.id;
  ...
  for v_post in
    select p.id from public.blog_posts p
    where p.status = 'scheduled' and p.scheduled_at <= now()
  loop
    begin
      v_post_revision := public.publish_blog_post(v_post.id);

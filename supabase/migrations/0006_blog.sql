...
create unique index blog_posts_slug_active_key on public.blog_posts (slug) where deleted_at is null;
...
create index blog_post_blocks_post_id_position_idx on public.blog_post_blocks (post_id, position);
...
alter table public.blog_posts
  add constraint blog_posts_published_revision_id_fkey
  foreign key (published_revision_id) references public.blog_post_revisions (id) on delete set null;
...
create policy "blog_categories_write" on public.blog_categories
  ...
  using (public.has_permission((select auth.uid()), 'blog.update'))
  with check (public.has_permission((select auth.uid()), 'blog.update'));
...
create policy "blog_posts_select_staff" on public.blog_posts
  ...
  using (public.has_permission((select auth.uid()), 'blog.view'));
create policy "blog_posts_insert" on public.blog_posts
  for insert to authenticated
  with check (public.has_permission((select auth.uid()), 'blog.create'));
create policy "blog_posts_update_content" on public.blog_posts
...
create policy "blog_post_blocks_select" on public.blog_post_blocks
  for select to authenticated
  using (public.has_permission((select auth.uid()), 'blog.view'));
create policy "blog_post_blocks_write" on public.blog_post_blocks
  for all to authenticated
  using (public.has_permission((select auth.uid()), 'blog.update'))
  with check (
    public.has_permission((select auth.uid()), 'blog.update')
    and (block_type <> 'custom_html' or public.has_permission((select auth.uid()), 'pages.custom_html'))
  );

create policy "blog_post_revisions_select_published" on public.blog_post_revisions
  for select to anon, authenticated
  using (
    exists (
      select 1 from public.blog_posts p
      where p.published_revision_id = blog_post_revisions.id and p.status = 'published'
    )
  );
create policy "blog_post_revisions_select_staff" on public.blog_post_revisions
  for select to authenticated
  using (public.has_permission((select auth.uid()), 'blog.view'));
create policy "blog_post_revisions_insert" on public.blog_post_revisions
  for insert to authenticated
  with check (public.has_permission((select auth.uid()), 'blog.publish'));

create policy "blog_post_tags_select" on public.blog_post_tags
  for select to anon, authenticated using (true);
create policy "blog_post_tags_write" on public.blog_post_tags
  for all to authenticated
  using (public.has_permission((select auth.uid()), 'blog.update'))
  with check (public.has_permission((select auth.uid()), 'blog.update'));
...
create or replace function public.publish_blog_post(p_post_id uuid, p_change_note text default null)
returns public.blog_post_revisions
language plpgsql
security definer
...
declare
  ...
  v_next_revision int;
  v_blocks jsonb;
  v_revision public.blog_post_revisions;
begin
  if (select auth.uid()) is not null
     and not public.has_permission((select auth.uid()), 'blog.publish') then
  ...
  from public.blog_post_revisions where post_id = p_post_id;

  select coalesce(jsonb_agg(to_jsonb(b) order by b.position), '[]'::jsonb) into v_blocks
  from public.blog_post_blocks b where b.post_id = p_post_id and not b.is_hidden;

  insert into public.blog_post_revisions (post_id, revision_number, title, blocks_snapshot, seo_snapshot, author_id, change_note)
  values (
...
create or replace function public.schedule_blog_post(p_post_id uuid, p_scheduled_at timestamptz)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if not public.has_permission((select auth.uid()), 'blog.publish') then
    raise exception 'Missing permission: blog.publish';
  end if;
  if p_scheduled_at <= now() then
    raise exception 'scheduled_at must be in the future';
  end if;
  update public.blog_posts set status = 'scheduled', scheduled_at = p_scheduled_at where id = p_post_id;
  insert into public.audit_logs (actor_id, actor_email, action, entity_type, entity_id, after)
...
create or replace function public.unpublish_blog_post(p_post_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.blog_posts set status = 'draft' where id = p_post_id;
...
create or replace function public.delete_blog_post(p_post_id uuid)
returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  if not public.has_permission((select auth.uid()), 'blog.update') then
    raise exception 'Missing permission: blog.update';
  end if;
  update public.blog_posts set deleted_at = now() where id = p_post_id;
  insert into public.audit_logs (actor_id, actor_email, action, entity_type, entity_id)
  values ((select auth.uid())::text, (select auth.jwt() ->> 'email'), 'blog_post.delete', 'blog_post', p_post_id::text);
end;
$$;

create or replace function public.restore_blog_post(p_post_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.blog_posts set deleted_at = null where id = p_post_id;
  insert into public.audit_logs (actor_id, actor_email, action, entity_type, entity_id)
